...
abcProc.q.old <- function(true.stats.ref, sim.stats.ref, sim.par.ref, par.true, 
	  bigRet = FALSE, tols = .01, quants = c(.025, .25, .5, .75, .975)){
...
avdists.r1 <- all.r1 %>% filter(str_detect(var, 's1|theta')) %>%
    left_join(par.true, by = 'var') %>% mutate(dist = abs(val - true)) %>%
        group_by(var) %>% summarize(avdist = median(dist))
avdists.r2 <- all.r2 %>% filter(var == "m") %>%
    left_join(par.true, by = 'var') %>% mutate(dist = abs(val - true)) %>%
    group_by(var) %>% summarize(avdist = median(dist))
...
toRet <- toRet %>%
    mutate(avdist = ifelse(var == "m", avdists.r2$avdist, avdists.r1$avdist))
