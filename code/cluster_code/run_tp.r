...
truths.sim <- foreach(s = par.true.full$s,
		      ...
		      n = par.true.full$n,
		      N = par.true.full$N,
                      offs = par.true.full$offs,
                      addhund = par.true.full$addhund,
    ...
    toRet <- subSim(N, N, trueMu, s, s, m, len, relgens = sampGens, recenterCond = NA) %>%
	     ...
	     mutate(N = N, n = n, s1 = s, m = m, i = i, runtype = runtype, offs = offs, addhund = addhund)
    toRet
 }                                                              
...
stat.true.full <- left_join(tbl_df(truths.sim), par.true.full,
          by = c("N", "n", "s1", "m", "i", "runtype", "offs", "addhund"))
...
foreach(i = 1:nrow(stat.true.full), .combine = "rbind")%do%{
    ...
    toPrint <- tbl_df(cbind(stats.to.add, abc.to.add))
     ...
     write.table(toPrint, "../../out/sumStats-varytp.txt", row.names = FALSE,
